...
- `src/pages/` - `index.astro` (the single-page homepage narrative), `about/index.astro`, `contact/index.astro`, `blog/index.astro` + `blog/[slug].astro`, `work/index.astro` (mounts `WorkGallery` + holds its per-project framing array) + `work/[slug].astro`, `lab/` (the R3F island sandbox / reference, unlinked from nav). `work/[slug].astro` groups a project's flat block array into labelled sections (kicker on the rail, prose on the body, photos full width as justified orientation-grouped rows) and closes on the `heroImage` studio shot; alongside the image blocks it renders a **`booklet`** block as the interactive 3D book **`BookletViewer.astro`** (a CSS-3D book - no WebGL: a spinnable closed cover carrying the PDF's own cover, leather-grained board + rounded corners + spine + striped page edges, that opens to a flippable two-page spread, X/Esc to close; PDF pages are rasterised on the client with pdf.js loaded lazily on scroll and windowed; theme rides the site tokens; the inline script is `is:inline` and all CSS is namespaced under `.cg-booklet` because the page leaves are built in JS) - reusable, e.g. for Wave's "Build a Speaker" manual. A booklet block inside a section lays out on the right of the grid, in line with that section's text: the section gets a `proj-sec--book` modifier that narrows the body track and drops the book into the right track (`work/[slug].astro`); on Wabi Sabi the booklet sits beside "The research". The served PDF lives in `public/` (the MUJI booklet is `public/wabi-sabi-booklet.pdf`).
