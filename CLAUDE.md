...
- **Work gallery** (`src/components/r3f/WorkGallery.tsx`) - **the `/work/` index**: a responsive grid (1→2→3→4 columns, mobile→4K) of the projects rendered as **grab-and-spin textured GLBs** on the plain white ground, each with a small "View project" link to `/work/[slug]/`. One R3F island mounted `client:visible`; each cell is its own `<Canvas>` that renders while on/near screen (IntersectionObserver, 300px pre-arm) and pauses off-screen, with fit-to-bounding-sphere + Meshy-normalise + in-engine studio-IBL handoff (the pattern inherited from the removed ProjectViewer carousel). Per-project framing (`fill`, resting `viewRY`/`viewRX`) lives in one array in `src/pages/work/index.astro` - `fill` is kept ≤1.0 so the sphere never clips at any spin angle, and each stage is a taller `4∶5` box for breathing room. Two gotchas baked in: the canvas is pulled **out of flow** (via the Canvas `style` prop → `position:absolute`, which beats R3F's inline `position:relative`) so its drawing-buffer height can't feed back into the `aspect-ratio` stage and inflate it frame-by-frame; and the gallery wrapper must **not** carry the editorial `.reveal` class (it races scroll-restoration and can stick at `opacity:0` - the island brings its own per-model GSAP entrance). Each model cell shows a faint, on-brand **loading skeleton** (`.wg-skeleton` - a slow tonal pulse on its `::before` so the wrapper's opacity stays free to fade) behind the transparent canvas; it fades out once the GLB is ready (`Model` fires `onReady`), so cells never flash empty on slow networks. The Nuclear Question is omitted until its model exists.
- **Project imagery** - project pages (`/work/[slug]`) render their imagery as plain responsive `<img>` (with `srcset`) from the manifest. One hard-won rule survives from the removed 3D-image-canvas experiment: an image frame must adopt the image's **true** aspect read from the decoded image, never a manifest value - manifest-aspect sizing squishes portrait/landscape. Don't reintroduce it.
- **Superseded components were removed in June 2026** (recoverable from git history): `Hero.astro` (the old standalone CG hero), `Carousel.astro` + `r3f/ProjectViewer.tsx` (the old Selected-Work carousel - its fit-scale + material-normalisation + studio-IBL logic lives on inside ScrollHero and WorkGallery), `WorkIndex.astro` (the old typographic `/work/` list) and `public/js/hero-canvas.js`/`object-canvas.js`. R3F is first-class for `/work/`, `/lab/` and future stateful-3D islands.
...
- **`data-theme` attribute on `<html>`** (`light` | `dark`) is the switch. A `:root[data-theme="dark"]` block in `tokens.css` overrides the colour tokens (`--bg`, `--text`, the quiet/faint/rule/hairline derivations, `--field-bg`, `--nav-backdrop`); everything reading those flips automatically. **The site ALWAYS opens in dark** (its signature ground): the toggle switches within the current page only and the choice is **deliberately NOT persisted** - every page load (including in-session navigations) starts dark again. `Nav.astro` even clears any legacy `cg-theme` localStorage value so an old stored "light" can't linger. (To remember the choice instead, restore the localStorage read in the Layout head script and a setItem in Nav - see the comments at both sites.) The starting dark palette (near-black `#0E0E0E` ground, warm near-white `#F4F3EF` ink) is a tunable call - adjust in `tokens.css`, not inline. The single accent is kept in both.
- **No-flash:** a blocking inline script at the very top of `<head>` (`Layout.astro`) sets `data-theme="dark"` before the stylesheets load, so there is never a flash of the wrong ground.
- **The hero:** the homepage particle hero is the only opaque WebGL surface, so it can't flip via CSS. The toggle dispatches a **`theme:change` CustomEvent** on `window` (mirroring the `hero:loaded` contract); `ScrollHero.astro` listens and runs `applyTheme()` - sets the clear/scene colour + the `uInk` particle uniform, and lazily builds the bloom composer (dark only). Particle **motion/density/behaviour is identical** in both themes; only ink colour (→white) and the dark-only bloom differ. The alpha `/work/` gallery canvases need no change - they inherit the page ground.
...
- `src/components/BookletViewer.astro` - the interactive **3D booklet** rendered for a project's `booklet` block (see the `work/[slug].astro` booklet-block note below). A CSS-3D book (no WebGL): spinnable closed cover carrying the PDF's own cover, leather-grained board + rounded corners + spine + striped page edges, opening to a flippable **two-page spread** (click an edge / drag a page / arrows), X or Esc to close, return-to-start at the end. Self-contained: an **`is:inline`** script (so Astro doesn't type-check/bundle it, matching the vanilla-script convention) and CSS **namespaced under `.cg-booklet`** in an `is:global` block because the page leaves are built in JS and would otherwise miss Astro's scope hash. Pages are rasterised on the client with **pdf.js** (CDN, dynamic-imported lazily the first time the book scrolls into view, then windowed so only a few decode at once); theme rides the site tokens. The no-build study is `prototypes/booklet/`. (Perf note: pages render at runtime; a build-time pre-render to WebP - mirroring `process-images`/`optimize-models` - is the obvious next step if the PDF download + client raster ever needs trimming.)
- `src/components/Grid.astro` - the column container (see grid section).
- `src/components/r3f/` - **React/R3F islands**. `WorkGallery.tsx` (the `/work/` model gallery - a grid of per-cell grab-and-spin GLB canvases; see Current state); `ProjectModel.tsx` (the project-page grab-and-spin GLB stage); `ModelBoundary.tsx` (shared error boundary - a failed GLB degrades quietly instead of a stuck skeleton/blank stage); `SpinCard.tsx` (minimal reference pattern). Mounted `client:visible`.
...
- `src/pages/` - `index.astro` (the single-page homepage narrative), `about/index.astro`, `contact/index.astro`, `blog/index.astro` + `blog/[slug].astro`, `work/index.astro` (mounts `WorkGallery` + holds its per-project framing array) + `work/[slug].astro`, `lab/` (the R3F island sandbox / reference, unlinked from nav). `work/[slug].astro` groups a project's flat block array into labelled sections (kicker on the rail, prose on the body, photos full width as justified orientation-grouped rows) and closes on the `heroImage` studio shot; alongside the image blocks it renders a **`booklet`** block as the interactive 3D book **`BookletViewer.astro`** (a CSS-3D book - no WebGL: a spinnable closed cover carrying the PDF's own cover, leather-grained board + rounded corners + spine + striped page edges, that opens to a flippable two-page spread, X/Esc to close; PDF pages are rasterised on the client with pdf.js loaded lazily on scroll and windowed; theme rides the site tokens; the inline script is `is:inline` and all CSS is namespaced under `.cg-booklet` because the page leaves are built in JS) - reusable, e.g. for Wave's "Build a Speaker" manual. A booklet block renders below its section's text, **left-aligned on the body track** so the closed book's left edge (and the open spread's) line up with the prose, and the spread fills rightward (`.proj-booklet` in `work/[slug].astro`); on Wabi Sabi the booklet sits under "The Brief". The served PDF lives in `public/` (the MUJI booklet is `public/wabi-sabi-booklet.pdf`).
...
- `prototypes/booklet/` - `index.html`, the no-build study where the 3D booklet was developed (the CSS-3D book + page-turn, fed `wabi-sabi-booklet.pdf` via pdf.js); the production version is `BookletViewer.astro`. No-build, safe to delete.
...
Two ways 3D ships:
1. **Vanilla three.js** via `<script is:inline type="module">`, resolving `three`/addons through the Layout import map (CDN, pinned `three@0.162.0`). This is what the homepage ScrollHero uses today (the old `public/js/` shared modules were removed with their consumers).
2. **React islands** - now **first-class on `main`** (the preferred path for richer, stateful 3D). The stack is installed and wired: `@astrojs/react` (in `astro.config.mjs`) + **react-three-fiber 9 + drei 10 + GSAP 3 (`@gsap/react`)** + npm `three@0.184`, with JSX in `tsconfig.json`. Components live in `src/components/r3f/`; mount with `client:visible` so editorial pages stay static. **`WorkGallery.tsx`** (the live `/work/` model gallery) and **`ProjectModel.tsx`** (the project-page model stage) are the real examples; **`SpinCard.tsx`** + the **`/lab/`** page are the minimal reference pattern - copy these to build new 3D sections. Conventions: studio IBL via in-engine `RoomEnvironment` (no HDRI download - deploy-safe), `useGSAP` for choreography, `OrbitControls` with zoom/pan disabled (so the wheel still scrolls), render loops pause off-screen. Convert vanilla pieces to R3F incrementally; don't rewrite wholesale (notably, leave the hero as vanilla).
