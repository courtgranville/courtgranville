...
- **Superseded but kept in tree:** `Hero.astro` (old standalone CG hero) and `Carousel.astro` + `src/components/r3f/ProjectViewer.tsx` (the old Selected-Work textured-GLB R3F carousel) are no longer used by the homepage. `ProjectViewer`'s fit-scale + material-normalisation + studio-IBL logic was the reference for ScrollHero's model handoff. R3F is still first-class for `/lab/` and future stateful-3D islands.
